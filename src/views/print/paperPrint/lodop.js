...
let amountLtoU = function (n) {
    ...
    if (!isNaN(IsNum)) {
        ...
        for (var i = 0; i < fraction.length; i++) {
            s += (digit[Math.floor(decimal(n * 100, 2) / 10 * Math.pow(10, i) % 10)] + fraction[i]).replace(/零./, '');
        }
        s = s || '整';
        n = Math.floor(n);
